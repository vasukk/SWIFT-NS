...
# Require the entire 200-nt window to align for classification
minimum_window_query_coverage = 100.0

# Plotting-only coverage threshold.
# This allows below-threshold identities to appear while still requiring
# most of each 200-nt window to align.
minimum_plot_window_query_coverage = 80.0
...
with tempfile.TemporaryDirectory(
    ...
    def get_best_hits(
        ...
        # Always require the requested query coverage.
        blast_df = blast_df[
            blast_df["query_coverage"]
            >= minimum_coverage
        ].copy()

        # Apply the identity threshold for classification results.
        # For plotting, keep below-threshold identities visible.
        if apply_identity_threshold:
            blast_df = blast_df[
                blast_df["identity"]
                >= minimum_identity
            ].copy()
    ...
    # Best hits for plotting only: retain below-threshold identities.
    window_plot_a = get_best_hits(
        window_a_output,
        "plot_A",
        minimum_plot_window_query_coverage,
        apply_identity_threshold=False
    )

    window_plot_b = get_best_hits(
        window_b_output,
        "plot_B",
        minimum_plot_window_query_coverage,
        apply_identity_threshold=False
    ...
    window_plot_a = window_plot_a.rename(
        columns={
            "query_id": "window_id"
        }
    )

    window_plot_b = window_plot_b.rename(
        columns={
            "query_id": "window_id"
        }
    )
    ...
    for sequence_id, group in window_results.groupby(
        ...
        # Use a fixed y-axis for all identity plots
        plt.ylim(
            0,
            100
        )
